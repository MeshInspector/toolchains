...
**Superseded — do not wire this into new images.** Use the `kisak-mesa` PPA, which
carries Mesa 26.1.7 for resolute on both amd64 and arm64.

Built to work around
[mesa#15660](https://gitlab.freedesktop.org/mesa/mesa/-/issues/15660): Ubuntu 26.04
ships Mesa 26.0.8, which mis-reads `gl_PrimitiveID` in a fragment shader that also
has user varyings and no geometry shader, so MeshInspector's picker got garbage
faces and crashed. This was built on the conclusion that no Mesa release carried
the fix — which was wrong. The stable branches cherry-picked it under their own
SHAs, so asking which tags contain the upstream commit reports none while the
released files do have it: `lp_state_derived.c` carries the fix from **26.1.6**
onward, and 26.1.5 does not. Check file contents, not commit SHAs.

The tarballs stay published so anything already referencing them keeps building.
The recipe is `.github/workflows/build-mesa-llvmpipe.yml` if a genuinely unfixed
Mesa ever needs the same treatment: llvmpipe only, built in an `ubuntu:26.04`
container against Resolute's own `llvm-21-dev`, 6 min on x64 and 3 min on arm64,
with a smoke test that refuses to publish a build still reporting the distro's
version.
